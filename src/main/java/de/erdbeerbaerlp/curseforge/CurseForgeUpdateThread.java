...
public class CurseForgeUpdateThread extends TimerTask {
    ...
    public CurseForgeUpdateThread(String id, EmbedMessage embedMessage, Config config)
        ...
        final Timer timer = new Timer("CurseForge Update Detector for %s (ID: %d)".
                formatted(this.proj.name(), this.proj.id()));
        timer.scheduleAtFixedRate(this, TimeUnit.SECONDS.toMillis(60),
    ...
    public void run() {
        try {
            ...
            if (debug) {
                System.out.printf("<%s> Cached: %d Newest:%d%n",
